...
class Ridge:
    def __init__(self, start_lag=0, end_lag=50, alpha=1, verbose=True, lagged_matrix_fill = 0):
        '''
        Class for performing ridge regression on a system response to an impulse.
        args:
        ...
        #allow only filling value of 0 or np.nan and give error otherwise
        assert lagged_matrix_fill in [0, np.nan], "filling_value must be 0 or np.nan"
        self.lagged_matrix_fill = lagged_matrix_fill
    ...
    def fit(self, X, y):
        ...
        for ipf in range(self.n_input_features):
            #print(self._get_lagged_matrix(X[:, ipf]))
    ...
    def _get_lagged_matrix(self, X):
        """gets lagged matrix for single feature X

        Args:
            X (np.array): feature vector of shape (n_samples, 1) 

        Returns:
            np.array: lagged matrix of shape (n_samples, n_lags)
        """

        if self.start_lag<=0:
            #Edit Constantin
            X_ = np.pad(X, (0, abs(self.start_lag)), 'constant', constant_values=self.lagged_matrix_fill)
            r = np.zeros(self.num_lags)
            if np.isnan(self.lagged_matrix_fill):
                r[:] = np.nan
            lagged_matrix = toeplitz(X_, r)
            #Edit Constantin
            lagged_matrix = lagged_matrix[abs(self.start_lag):,:]
            return lagged_matrix
        
        if self.start_lag>0:
            if self.lagged_matrix_fill == 0:
                r = np.zeros((self.num_lags+self.start_lag))
            elif np.isnan(self.lagged_matrix_fill):
                r = np.zeros((self.num_lags+self.start_lag))
                r[:] = np.nan
